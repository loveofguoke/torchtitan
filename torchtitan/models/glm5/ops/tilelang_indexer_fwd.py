...
# See TILELANG_LICENSE in this directory.
...
"""TileLang forward kernel for exact GLM-5 DSA index scores.

Each kernel tile computes per-head ``Q @ K.T``, applies ReLU and the
query-specific head weights, then reduces the head dimension. A second kernel
masks keys outside each packed query's legal ``[start, end)`` interval. The
Python operator runs this kernel in query blocks before exact top-k selection.
"""
...
def indexer_fwd_interface(q, kv, weights, cu_seqlen_ks, cu_seqlen_ke, clean_logits=True):
    """Run score computation followed by packed-range mask cleanup."""
