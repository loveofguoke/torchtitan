...
"""TileLang backward kernel for top-k absorbed SparseMLA.

The kernel reconstructs selected attention probabilities from LSE, computes
dQ, and atomically accumulates the latent dK+dV and RoPE-tail dK gradients into
the compressed KV tensor.
"""
...
def sparse_mla_bwd(q, kv, o, do, indices, lse, sm_scale=None, is_casual=True, return_kernel=False, delta=None):
    """Run the batch-one token-first SparseMLA backward kernels."""
    q = q.unsqueeze(0)
    kv = kv.unsqueeze(0)
    o = o.unsqueeze(0)
    do = do.unsqueeze(0)
